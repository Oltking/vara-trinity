...
exports.runPulseDao = runPulseDao;
...
function getAgentTags(agent) {
    ...
    if (/oracle|price|data|feed/i.test(agent.description))
        tags.push('data', 'oracle');
    if (/market|predict|trade|bet/i.test(agent.description))
        tags.push('markets', 'prediction');
    if (/social|chat|board|community/i.test(agent.description))
        tags.push('social');
    if (/workflow|automate|schedule|trigger/i.test(agent.description))
        tags.push('automation');
    if (/creative|content|pulse|generate/i.test(agent.description))
        tags.push('creative');
    if (/bounty|reward|escrow|fund/i.test(agent.description))
        tags.push('bounty');
    if (/strategy|analyze|signal|insight/i.test(agent.description))
        tags.push('strategy');
    if (/game|match|tic-tac|arena/i.test(agent.description))
        tags.push('gaming');
    return tags;
}
function fetchAgents() {
    ...
    try {
        const result = (0, child_process_1.execSync)(`vara-wallet --account ${config_1.config.ACCT} --network ${config_1.config.VARA_NETWORK} ` +
            ...
            `--args '[{"include": null}, null, 100]' --idl "${config_1.config.A2A_IDL}"`, { timeout: 20_000, encoding: 'utf-8' });
        const items = JSON.parse(result)?.result?.items || [];
        return items.map((a) => ({
...
function computeMatches(agents) {
    const scores = [];
...
function generateAgentReport(agent, matches) {
...
function runPulseDao() {
    if (!config_1.config.NETWORK_PID || !config_1.config.A2A_IDL)
        return;
    const agents = fetchAgents();
    if (agents.length < 2) {
        console.log('PulseDAO: not enough agents');
        return;
    }
    const ourPids = new Set([config_1.config.BRIDGE_PID, config_1.config.FLOW_PID, config_1.config.PULSE_PID, config_1.config.STRATEGY_PID].filter(Boolean));
    ...
    // Post to Board
    const argsFile = (0, path_1.join)((0, os_1.tmpdir)(), `pulse-dao-${Date.now()}.json`);
    (0, fs_1.writeFileSync)(argsFile, JSON.stringify([config_1.config.PULSE_PID, {
            title: `Pulse DAO — ${agents.length} agents analyzed, ${matches.length} matches found`,
            body,
            tags: ['pulse-dao', 'matchmaking', 'network-analysis'],
        }]), 'utf-8');
    try {
        (0, child_process_1.execSync)(`vara-wallet --account ${config_1.config.ACCT} --network ${config_1.config.VARA_NETWORK} ` +
            ...
            `--args-file ${argsFile} --idl "${config_1.config.A2A_IDL}"`, { timeout: 20_000, encoding: 'utf-8' });
...
//# sourceMappingURL=pulse-dao.js.map